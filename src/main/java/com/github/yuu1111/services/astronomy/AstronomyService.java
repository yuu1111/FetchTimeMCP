...
/**
 * 天文学的情報サービス 日の出・日の入り、月相などの天文情報を計算
 */
public class AstronomyService {
  ...
  public AstronomicalInfo getAstronomicalInfo(double latitude, double longitude, LocalDate date) {
    logger.debug("Calculating astronomical info for lat:{}, lon:{}, date:{}", latitude, longitude,
        date);
   ...
   * 太陽情報を計算
   */
  private void calculateSunInfo(AstronomicalInfo info, double latitude, double longitude,
      LocalDate date) {
    try {
      // 日の出・日の入り計算
   ...
   * トワイライト（薄明）時刻を計算
   */
  private void calculateTwilight(AstronomicalInfo info, double latitude, double longitude,
      LocalDate date) {
    try {
      // 市民薄明
      SunTimes civilTwilight = SunTimes.compute().on(date).at(latitude, longitude)
      ...
      // 航海薄明
      SunTimes nauticalTwilight = SunTimes.compute().on(date).at(latitude, longitude)
      ...
      // 天文薄明
      SunTimes astronomicalTwilight = SunTimes.compute().on(date).at(latitude, longitude)
   ...
   * 月情報を計算
   */
  private void calculateMoonInfo(AstronomicalInfo info, double latitude, double longitude,
      LocalDate date) {
    try {
      // 月の出・月の入り
      ...
      // 月相計算
      MoonPhase moonPhase = MoonPhase.compute().on(date).execute();
      ...
      // 月の距離
      MoonPosition moonPosition = MoonPosition.compute().on(date.atStartOfDay(ZoneId.of("UTC")))
  ...
  private MoonPhaseType calculateMoonPhaseType(MoonPhase moonPhase) {
    ...
    // ここでは照度ベースで判定
    MoonIllumination illumination = MoonIllumination.compute().on(nextNewMoon.toLocalDate())
        .execute();
   ...
   * 太陽位置を計算
   */
  private void calculateSolarPosition(AstronomicalInfo info, double latitude, double longitude,
      LocalDate date) {
    try {
      // 正午の太陽位置を計算
      ...
      SunPosition sunPosition = SunPosition.compute().on(zonedNoon).at(latitude, longitude)
          .execute();
