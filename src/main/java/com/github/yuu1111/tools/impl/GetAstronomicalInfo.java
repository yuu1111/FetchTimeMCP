...
public class GetAstronomicalInfo implements MCPTool {
    ...
    public ToolInfo getInfo() {
        return ToolInfo.builder()
            .name(getName())
            .description("Get astronomical information for a specific location and date")
            .inputSchema(Map.of(
                "type", "object",
                "properties", Map.of(
                    "latitude", Map.of(
    ...
    public Object execute(Map<String, Object> parameters) throws MCPError {
        ...
        if (latitude == null) {
            throw MCPError.invalidParams("latitude parameter is required");
        }
        if (longitude == null) {
            throw MCPError.invalidParams("longitude parameter is required");
        }
        ...
        if (latitude < -90 || latitude > 90) {
            throw MCPError.invalidParams("latitude must be between -90 and 90");
        }
        if (longitude < -180 || longitude > 180) {
            throw MCPError.invalidParams("longitude must be between -180 and 180");
        }
        ...
        try {
            ...
            throw MCPError.internalError("Failed to get astronomical info: " + e.getMessage());
        }
    }
